    ...
    <div class="container mt-5" style="padding: 50px;">
        <div class="text-center">
            @if ($order_success === 1)
                ...
                <p class="lead">Đơn hàng <strong>#{{ $maHD }}</strong> của bạn đã được xử lý thành công. Chúng tôi
                    rất vui khi có bạn là khách hàng của mình!</p>
                <div class="mt-4">
            ...
            @elseif($order_success === 0)
                ...
                <p class="lead">Rất tiếc, giao dịch cho đơn hàng <strong>#{{ $maHD }}</strong> của bạn không được
                    hoàn tất. Bạn có muốn thử lại với phương thức thanh toán khác?</p>
                ...
                <form id="changePaymentMethodForm" action="{{ route('payment.updatePaymentMethod') }}" method="POST">
                    ...
                    <button type="button" class="btn btn-primary" id="changeToCODBtn">Đổi sang thanh toán khi nhận hàng
                        COD?</button>
                </form>
            ...
            @elseif(session('order_success') === 2)
                ...
                <p class="mt-3">Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi. Chúng tôi sẽ tiếp tục xử lý đơn hàng của bạn.
                </p>
                <a href="{{ route('user') }}" class="btn btn-primary btn-lg mt-4">Trở về trang chủ</a>
            @else
                <!-- Nếu không xác định trạng thái thanh toán -->
                <h1 class="display-4 text-warning">Lỗi hệ thống!</h1>
                <p class="lead">Có lỗi xảy ra trong quá trình thanh toán. Vui lòng thử lại sau hoặc liên hệ với chúng tôi
                    để được hỗ trợ.</p>
