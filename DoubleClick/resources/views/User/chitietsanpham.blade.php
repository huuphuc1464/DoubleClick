                ...
                {{-- <form action="{{ route('danhgia.store') }}" method="POST"> --}}
                    ...
                    <div class="rating">
                        ...
                        <div class="rate-container">
                            @for ($i = 1; $i <= 5; $i++)
                                <input type="radio" id="star{{ $i }}" name="SoSao"
                                    value="{{ $i }}">
    ...
    <div class="author-products">
        ...
        <div class="flex-wrap product-list d-flex justify-content-center">
            @foreach ($relatedProducts as $sach)
                <div class="m-3 text-center product-item card"
                    onclick="window.location='{{ route('product.detail', $sach->MaSach) }}'"
                    style="cursor: pointer; width: 200px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    <div class="card-body">
                        <img src="{{ asset('img/sach/' . $sach->AnhDaiDien) }}" alt="{{ $sach->TenSach }}"
                            class="card-img-top" style="width: 150px; height: 200px; object-fit: cover;">
