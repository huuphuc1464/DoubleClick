...
<div class="container mt-4 mb-5">
    ...
    <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
            <a class="nav-link {{ request('tab', 'dang-ban') === 'dang-ban' ? 'active' : '' }}" href="?tab=dang-ban" role="tab">Các sách đang bán</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link {{ request('tab') === 'ngung-ban-books' ? 'active' : '' }}" href="?tab=ngung-ban-books" role="tab">Sách đã ngưng bán</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link {{ request('tab') === 'sap-het-hang' ? 'active' : '' }}" href="?tab=sap-het-hang" role="tab">Sách sắp hết hàng</a>
        </li>
    </ul>

    <div class="tab-content" id="myTabContent">
        <!-- Các sách đang bán -->
        <div class="tab-pane fade {{ request('tab', 'dang-ban') === 'dang-ban' ? 'show active' : '' }}" id="dang-ban" role="tabpanel" aria-labelledby="dang-ban-tab">
            <div class="card">
            ...
            <div class="mt-3 d-flex justify-content-end">
                ...
                <ul class="pagination">
                    <li class="page-item {{ ($sach->currentPage() == 1) ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $sach->url(1) }}&tab=dang-ban" aria-label="First">Trang đầu</a>
                    </li>
                    @if ($sach->currentPage() > 1)
                    <li class="page-item">
                        <a class="page-link" href="{{ $sach->url($sach->currentPage() - 1) }}&tab=dang-ban">{{ $sach->currentPage() - 1 }}</a>
                    </li>
                    @endif
                    ...
                    @if ($sach->currentPage() < $sach->lastPage())
                        ...
                        <li class="page-item {{ ($sach->currentPage() == $sach->lastPage()) ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $sach->url($sach->lastPage()) }}&tab=dang-ban" aria-label="Last">Trang cuối</a>
                        </li>
                </ul>
                @endif
            </div>
            ...
            <div class="mt-3 d-flex justify-content-end">
                ...
                <ul class="pagination">
                    <li class="page-item {{ ($ngungban->currentPage() == 1) ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $ngungban->url(1) }}&tab=ngung-ban-books" aria-label="First">Trang đầu</a>
                    </li>
                    @if ($ngungban->currentPage() > 1)
                    <li class="page-item">
                        <a class="page-link" href="{{ $ngungban->url($ngungban->currentPage() - 1) }}&tab=ngung-ban-books">{{ $ngungban->currentPage() - 1 }}</a>
                    </li>
                    @endif
                    <li class="page-item active">
                    ...
                    @if ($ngungban->currentPage() < $ngungban->lastPage())
                        ...
                        <li class="page-item {{ ($ngungban->currentPage() == $ngungban->lastPage()) ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $ngungban->url($ngungban->lastPage()) }}&tab=ngung-ban-books" aria-label="Last">Trang cuối</a>
                        </li>
                </ul>
                @endif
            </div>
        ...
        <!-- Sách sắp hết hàng -->
        <div class="tab-pane fade {{ request('tab') === 'sap-het-hang' ? 'show active' : '' }}" id="sap-het-hang" role="tabpanel" aria-labelledby="sap-het-hang-tab">
            <div class="card">
            ...
            <div class="mt-3 d-flex justify-content-end">
                ...
                <ul class="pagination">
                    <li class="page-item {{ ($hethang->currentPage() == 1) ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $hethang->url(1) }}&tab=sap-het-hang" aria-label="First">Trang đầu</a>
                    </li>
                    @if ($hethang->currentPage() > 1)
                    <li class="page-item">
                        <a class="page-link" href="{{ $hethang->url($hethang->currentPage() - 1) }}&tab=sap-het-hang">{{ $hethang->currentPage() - 1 }}</a>
                    </li>
                    @endif
                    <li class="page-item active">
                    ...
                    @if ($hethang->currentPage() < $hethang->lastPage())
                        ...
                        <li class="page-item {{ ($hethang->currentPage() == $hethang->lastPage()) ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $hethang->url($hethang->lastPage()) }}&tab=sap-het-hang" aria-label="Last">Trang cuối</a>
                        </li>
                </ul>
                @endif
            </div>
