                ...
                <div class="subcontent" style="flex: 1 0 75%; padding: 15px;">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse ($listCateBlog as $index => $category)
                                    <tr>
                                        <td class="text-center" style="border: 2px solid LightGray; ">{{ $index + 1 }}</td>
                                        <td style="border: 2px solid LightGray; ">{{ $category->TenDanhMucBlog }}</td>
                                        ...
                                        <td class="text-center" style="border: 2px solid LightGray; ">
                                            <div class="btn-action">
                                                @if($category->TrangThai == 1 || $category->TrangThai == 0)
                                                    <a href="{{ route('admin.category.edit', $category->MaDanhMucBlog) }}"
                                                        class="btn btn-primary btn-sm">Sửa</a>

                                                    <a href="{{ route('admin.category.delete', $category->MaDanhMucBlog) }}"
                                                        class="btn btn-danger btn-sm"
                                                        onclick="return confirm('Bạn có chắc chắn muốn xóa danh mục {{$category->TenDanhMucBlog}}?')">
                                                        Xóa
                                                    </a>
                                                @elseif($category->TrangThai == 2)
                                                    <a href="{{ route('admin.category.restore', $category->MaDanhMucBlog) }}"
                                                        class="btn btn-success"
                                                        onclick="return confirm('Bạn có chắc chắn muốn khôi phục danh mục {{$category->TenDanhMucBlog}}?')">
                                                        Khôi phục
                                                    </a>
                                                @endif
                                            </div>
                    ...
                    <div class="mt-3">
                        {{ $listCateBlog->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
